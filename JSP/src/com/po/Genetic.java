...
public class Genetic {
    ...
    private static void insert(int [] opt,int [] solution)
    {
        ...
        int [] offspring=new int[codeLength];
        while(a==b){
...
//        System.out.println(" b: "+b+"\n");
        //-test
        //将alreadyAddNum数组全部设为0;
        clearAlreadyAddNum();
        //将offspring的i<=a和i>=b的位全部设为solution[i];
        for(int i=0;i<=a;++i){
        ...
        //将a<i<b的区间内设为最优值的剩下的代码
        int indexopt=0;
        for(int i=a+1;i<b;++i){
            //System.out.println("length:"+opt.length+"indexopt"+indexopt);
            while(alreadyAddNum[opt[indexopt]]>=10){
                //System.out.println("length:"+opt.length+"indexopt"+indexopt);
                ++indexopt;
            }
            solution[i]=opt[indexopt];
            ++alreadyAddNum[opt[indexopt]];
            ++indexopt;
        }
    ...
    //将copy数组拷贝到arr
    private void copyArray(int [] copy,int [] arr){
        /*test*/
...
//        killAndGenerate();
    ...
    private int [] opxCrossOver(int [] so1,int [] so2){
        ...
        for(;i<codeLength;++i){
            //循环内的这一段代码在pbx中也出现了
            while(alreadyAddNum[so2[k]]>=10)k++;
            offspring[i]=so2[k];
            ++alreadyAddNum[so2[k]];
            ++k;
        }
    ...
    public void runAlgorithm(){
        for(int i=0;i<Parameter.numOfIteration;++i){
            oneGeneration();
            System.out.println("最小时间："+pop[0].time+"(第"+i+"次迭代)");
        }
    }
    ...
    private static void testso(int [] so){
        clearAlreadyAddNum();
        for(int i=0;i<so.length;++i){
            ++alreadyAddNum[so[i]];
        }
        for(int i=0;i<alreadyAddNum.length;++i){
            if(alreadyAddNum[i]!=10)System.out.println("出现错误");
        }
